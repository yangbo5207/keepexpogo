...
function StaggerItem({ label, index, playId }: StaggerItemProps) {
  ...
  return (
    <Animated.View
      className="flex-row items-center justify-between rounded-xs bg-cream-100 px-4 py-3 dark:bg-night-700"
      style={animatedStyle}
    >
      <View className="flex-1">
        <Text className="text-sm font-semibold text-cream-900 dark:text-cream-100">
          {label}
        </Text>
        <Text className="mt-1 text-xs text-cream-600 dark:text-cream-500">
          Delay {index * 100}ms
        </Text>
      </View>
      <View className="h-8 w-8 items-center justify-center rounded-xs bg-warning-100 dark:bg-warning-800/30">
        <Text className="text-xs font-bold text-warning-600 dark:text-warning-300">
          {index + 1}
        </Text>
      </View>
    </Animated.View>
